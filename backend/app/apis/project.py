...
class ProjectAPI(MoeAPIView):
    ...
    def put(self, project: Project):
        ...
        # 移动项目集仅限项目管理员与创建人
        if "project_set" in data:
            move_role = self.current_user.get_role(project)
            move_role_sys = move_role.system_code if move_role else ""
            if move_role_sys not in ("admin", "creator"):
                raise NoPermissionError(
                    gettext("仅项目管理员与创建人可移动项目集")
                )
        project.update(**data)
        project.reload()
        return {
...
class ProjectTargetOutputListAPI(MoeAPIView):
    ...
    def post(self, project: Project, target: Target):
        ...
        # 自动记录本次导出涉及的图片文件的嵌字者为当前用户
        files = project.files(
            type_only=FileType.IMAGE,
            file_ids_include=data.get("file_ids_include") or [],
            file_ids_exclude=data.get("file_ids_exclude") or [],
        )
        for f in files:
            f.update(typesetter=self.current_user.name)
        output_project(str(output.id))
