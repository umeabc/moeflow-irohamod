...
function* createSourceWorker(action: ReturnType<typeof createSourceSaga>) {
  ...
  try {
    ...
    // 标号成功后，自动记录翻译者为当前用户
    const me: UserState = yield select((state: AppState) => state.user);
    if (me && (me.name || me.id)) {
      try {
        yield api.file.editFile({ id: fileID, data: { translator: me.name || me.id } });
      } catch (e) {
        // 忽略：不阻塞标号流程
      }
    }
...
function* batchSelectTranslationWorker(
  ...
  try {
    ...
    // 校对确认后，将当前用户记录为该图片文件的校对者
    if (currentUser && (currentUser.name || currentUser.id)) {
      try {
        yield api.file.editFile({
          id: fileID,
          data: { proofreader: currentUser.name || currentUser.id },
        });
      } catch (e) {
        // 忽略：不阻塞批量选择流程
      }
