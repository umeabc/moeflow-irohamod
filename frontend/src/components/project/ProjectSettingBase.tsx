...
export const ProjectSettingBase: FC<ProjectSettingBaseProps> = ({
  ...
  const [projectSetOptions, setProjectSetOptions] = useState<
    { label: string; value: string }[]
  >([]);
  const [projectSetsLoading, setProjectSetsLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((result) => {
        const options = (result.data as any[]).map((item) => ({
          label: item.default
            ? formatMessage({ id: 'projectSet.default' })
            : item.name,
          value: item.id,
        }));
        setProjectSetOptions(options);
      })
      .catch((error) => {
        error.default();
      })
      .finally(() => {
        setProjectSetsLoading(false);
      });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentTeam?.id]);

  /** 移动项目到另一个项目集 */
  const moveProjectToProjectSet = (projectSetID: string) => {
    const target = projectSetOptions.find((o) => o.value === projectSetID);
    Modal.confirm({
      title: formatMessage({
        id: 'project.moveToProjectSet',
        defaultMessage: '移动项目集',
      }),
      content: formatMessage(
        {
          id: 'project.moveToProjectSetTip',
          defaultMessage:
            '确定要把项目『{project}』移动到项目集『{projectSet}』吗？',
        },
        { project: currentProject.name, projectSet: target?.label ?? '' },
      ),
      okText: formatMessage({ id: 'form.ok' }),
      cancelText: formatMessage({ id: 'form.cancel' }),
      onOk: () => {
        return api
          .editProject({ id: currentProject.id, data: { projectSet: projectSetID } })
          .then((result) => {
